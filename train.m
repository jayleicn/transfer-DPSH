function [net, U, B, loss_iter] = train (X1, L1, U, B, net, net_source, t, lambda, eta, iter, lr, loss_iter) 
    N = size(X1,4); % 5000
    batchsize = 64;
    index = randperm(N);
    for j = 0:ceil(N/batchsize)-1
        ...
        S = calcNeighbor (L1, ix, 1:N); % #batchsize * N in {1,0}, similarity matrix
        %% load and preprocess an image -- a batch of images
        im = X1(:,:,:,ix);
        ...
        res_source = vl_simplenn(net_source, im_);
